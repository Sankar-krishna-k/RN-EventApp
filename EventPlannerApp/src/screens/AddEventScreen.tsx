...
const handleSave = async () => {
  ...
  try {
    ...
    if (!user) return Alert.alert('No logged-in user found');
    ...
    if (existingEvent) {
      events = events.map((e) =>
        e.id === existingEvent.id
          ? { ...e, title, date: finalDate.toISOString(), category, userId: user.id }
          : e
      );
    } else {
  ...
  return (
    <View style={{ flex: 1 }}>
      ...
      {/* ✅ Success overlay with image */}
      {showSuccess && (
        <View style={styles.successOverlay}>
          <Image
            source={require('../assets/success.png')} // put your fun image in assets
...
const styles = StyleSheet.create({
  ...
  header: { fontSize: 26, fontWeight: '700', color: '#1C1C1E', marginBottom: 20 },
  card: { borderRadius: 12, padding: 0 },
  input: { marginTop: 12, backgroundColor: 'white' },
  label: { marginTop: 16, fontSize: 14, fontWeight: '600', color: '#6C6C70' },
  categoryContainer: { flexDirection: 'row', flexWrap: 'wrap', marginTop: 6 },
  categoryChip: { marginRight: 10, marginBottom: 10 },
  categoryChipSelected: { backgroundColor: '#007AFF', borderColor: '#007AFF' },
  categoryTextSelected: { color: '#fff', fontWeight: '600' },
  saveButton: { marginTop: 30, paddingVertical: 6 },
  ...
  successImage: { width: 120, height: 120, marginBottom: 10 },
  successText: { fontSize: 16, fontWeight: '600', color: '#28a745' },
});
